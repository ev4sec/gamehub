/**
 * Snake's engine invariants. Moved here verbatim when the suite grew a second
 * game; the assertions are unchanged from when they were the whole of smoke.ts.
 */
...
function invariants(s: GameState, label: string) {
  ...
  for (const p of s.snake) {
    ...
    expect(!seen.has(k), `${label}: snake overlaps itself at ${p.x},${p.y} (tick ${s.tick})`);
    seen.add(k);
    expect(
  ...
  for (const f of s.food) {
    ...
    expect(!seen.has(k), `${label}: food spawned on the snake at ${f.x},${f.y}`);
    expect(!s.walls.has(k), `${label}: food spawned inside a wall at ${f.x},${f.y}`);
    expect(!foodSeen.has(k), `${label}: two apples on the same cell`);
    foodSeen.add(k);
  }
  for (const d of s.drops) {
    expect(!s.walls.has(cellKey(d.pos.x, d.pos.y)), `${label}: power-up inside a wall`);
  }
  if (s.rival) {
    const rSeen = new Set<number>();
    for (const p of s.rival) {
      const k = cellKey(p.x, p.y);
      expect(!rSeen.has(k), `${label}: rival overlaps itself at ${p.x},${p.y}`);
      expect(!s.walls.has(k), `${label}: rival inside a wall at ${p.x},${p.y}`);
      rSeen.add(k);
    }
  }
  expect(tickInterval(s) > 0, `${label}: non-positive tick interval`);
}

const DIRS = ['up', 'down', 'left', 'right'] as const;

export function snakeEngineChecks(): void {
  ...
  // 2. Reversal is impossible no matter how fast you press.
  {
    ...
    step(s);
    expect(s.status === 'playing', 'reversal test: died on the first tick from a double-tap turn');
    expect(s.dir === 'up', `reversal test: expected to commit 'up' first, got '${s.dir}'`);
    step(s);
    expect(s.status === 'playing', 'reversal test: died on the second tick');
    expect(s.dir === 'left', `reversal test: expected 'left' on tick two, got '${s.dir}'`);
    expect(s.snake.length === before, 'reversal test: length changed unexpectedly');
  }
  ...
  // 3. A tight turn onto the cell the tail is vacating is legal.
  {
    ...
    queueDir(s, 'down');
    step(s);
    expect(s.status === 'playing', 'tail-chase test: died turning onto the vacating tail cell');
  }
  ...
  // 4. Eating grows by exactly one and never leaves food on the body.
  {
    ...
    const before = s.snake.length;
    step(s);
    expect(s.snake.length === before + 1, `eat test: length ${s.snake.length}, expected ${before + 1}`);
    expect(s.apples === 1, 'eat test: apple not counted');
    expect(s.score > 0, 'eat test: no score awarded');
    step(s);
    expect(s.snake.length === before + 1, 'eat test: length changed again on the following tick');
  }

  // 5. Every maze level is well formed and its start is not inside a wall.
  LEVELS.forEach((lvl, i) => {
    expect(!lvl.walls.has(cellKey(lvl.start.x, lvl.start.y)), `level ${i} (${lvl.name}): start is inside a wall`);
    for (let b = 1; b < 3; b++) {
      expect(
    ...
    for (const p of lvl.portals) {
      expect(!lvl.walls.has(cellKey(p.a.x, p.a.y)), `level ${i}: portal A inside a wall`);
      expect(!lvl.walls.has(cellKey(p.b.x, p.b.y)), `level ${i}: portal B inside a wall`);
    }
  });
  ...
  // 6. Speed actually ramps, and slow-mo actually slows.
  {
    ...
    expect(fast < fresh, `ramp test: interval did not drop (${fresh} -> ${fast})`);
    s.effects.push({ kind: 'slow', ticks: 30 });
    expect(tickInterval(s) > fast, 'slow-mo test: interval did not increase');
  }
  ...
  //    spawning, power-up drops, portals, level clears) actually execute.
  {
    ...
    expect((tally.eat ?? 0) > 500, 'AI soak: barely any apples eaten, the brain is not working');
    expect((tally.power ?? 0) > 0, 'AI soak: no power-up was ever collected');
    expect((tally.hazard ?? 0) > 0, 'AI soak: hazards never spawned in Endless');
    expect((tally.portal ?? 0) > 0, 'AI soak: portals were never used');
    expect((tally.levelUp ?? 0) > 0, 'AI soak: no maze level was ever cleared');
    expect((tally.rivalDown ?? 0) > 0, 'AI soak: the rival never went down');
  }
}
