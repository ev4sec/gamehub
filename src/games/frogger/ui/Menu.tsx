  ...
  return (
    <div className="w-full max-w-2xl px-6 py-10">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-5xl font-black tracking-tight text-lime-300">FROGGER</h1>
          <p className="mt-2 text-sm text-slate-400">
            {save.runs > 0
              ? `${save.runs} run${save.runs === 1 ? '' : 's'}, ${save.lifetimeHomes.toLocaleString()} frogs home, level ${save.bestLevel} at best.`
              : 'Six lanes of traffic, then a river with nothing to stand on but what floats past.'}
          </p>
        </div>
        <button
        ...
        {MODES.map((mode) => (
          <li key={mode}>
            <button
              ...
              className="w-full rounded-2xl border border-slate-700/70 bg-slate-800/40 p-5 text-left transition hover:border-lime-400/70 hover:bg-slate-800/80 focus:outline-none focus-visible:ring-2 focus-visible:ring-lime-400"
            >
      ...
      <div className="mt-8 rounded-xl border border-slate-800 bg-slate-900/40 p-4 text-xs leading-relaxed text-slate-500">
        ...
        to hop. On a touchscreen, swipe anywhere on the board, or just tap it to hop
        forward. P pauses, Escape leaves.
      </div>
