  ...
  return (
    <div className="w-full max-w-2xl px-6 py-10">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-5xl font-black tracking-tight text-rose-300">
            MISSILE COMMAND
          </h1>
          <p className="mt-2 text-sm text-slate-400">
            {save.runs > 0
              ? `${save.runs} run${save.runs === 1 ? '' : 's'}, ${save.lifetimeKills.toLocaleString()} intercepted, wave ${save.bestWave} at best.`
              : 'The cities cannot shoot back. You are the only thing between them and the sky.'}
          </p>
        </div>
        <button
        ...
        {MODES.map((mode) => (
          <li key={mode}>
            <button
              ...
              className="w-full rounded-2xl border border-slate-700/70 bg-slate-800/40 p-5 text-left transition hover:border-rose-400/70 hover:bg-slate-800/80 focus:outline-none focus-visible:ring-2 focus-visible:ring-rose-400"
            >
      ...
      <div className="mt-8 rounded-xl border border-slate-800 bg-slate-900/40 p-4 text-xs leading-relaxed text-slate-500">
        <span className="font-semibold text-slate-400">Controls.</span> Tap or click
        where you want the explosion. The nearest battery with ammo fires, so one
        finger is the whole game. P pauses, Escape leaves.
      </div>
